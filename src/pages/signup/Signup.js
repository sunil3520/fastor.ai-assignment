...
const Signup = () => {
  ...
  const handleSignupAndLogin = () => {
    if (!isSignIn) {
      dispatch(signup({ phone: phoneNumber, dial_code: dialCode })).then(
        (res) => {
          if (res?.status == "Success") {
            setIsSignIn(true);
            showToast('Otp has been sent', 'success');
    ...
    if (isSignIn) {
      window.alert("Enter 123456 pin")
      const otp = otpValues.join("");
      dispatch(login({ phone: "1236547890", otp: otp, dial_code: dialCode }))
      .then((res)=>{
        if(res?.status === "Success"){
          navigate("/products")
        }
  ...
  return (
    <div className="h-[100vh] flex flex-col justify-center items-center">
      <div className=" w-[95%] flex flex-col items-center">
        <Label isSignIn={isSignIn} />
        <div className="mt-[30px] w-[95%] m-[auto]">
          {!isSignIn ? (
            <div className="flex">
              <input
                type="text"
      ...
      <Button handleSignupAndLogin={handleSignupAndLogin}>
        {isSignIn ? "Verify" : "Send Code"}
      </Button>
      {isSignIn && (
        <p className="mt-5 font-Urnbanist text-[15px] font-semibold">
